...
const App = () => {
  ...
  const generateReport = async (record) => {
    ...
    try {
      ...
      // Share option dikhana (WhatsApp etc)
      await Share.share({
      ...
      // Browser fallback (Agar mobile pe nahi hai)
  ...
  const handleSubmission = () => {
    ...
    generateReport(newRecord); // This will save and share
            ...
            <div className="grid grid-cols-2 gap-4">
              {departments.map((dept) => (
                <button key={dept.id} onClick={() => setSelectedDept(dept.id)} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col items-center gap-3 active:scale-95 transition-all">
                  <div className={`w-16 h-16 ${dept.color} rounded-3xl flex items-center justify-center`}>
                    <dept.icon size={28} />
                  </div>
                  <span className="font-black text-slate-800 text-[11px] uppercase">{dept.name}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-3">
               {STUDENT_DATABASE[selectedClass]
                ...
                .map(s => (
                  <div key={s.roll} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center justify-between active:bg-slate-50">
                    <div className="flex-1 truncate pr-4">
                      <div className="text-[9px] font-black text-red-700 uppercase">{s.roll}</div>
                      <div className="font-extrabold text-slate-900 text-lg leading-tight">{s.name}</div>
                    </div>
                    <div className="flex gap-3">
                       <button onClick={() => setAttendance(prev => ({ ...prev, [s.roll]: 'Present' }))} className={`w-14 h-14 rounded-2xl font-black transition-all border-2 flex items-center justify-center text-xl ${attendance[s.roll] === 'Present' ? 'bg-emerald-600 border-emerald-700 text-white shadow-lg' : 'bg-slate-50 border-slate-100 text-slate-300'}`}>P</button>
                       <button onClick={() => setAttendance(prev => ({ ...prev, [s.roll]: 'Absent' }))} className={`w-14 h-14 rounded-2xl font-black transition-all border-2 flex items-center justify-center text-xl ${attendance[s.roll] === 'Absent' ? 'bg-rose-600 border-rose-700 text-white shadow-lg' : 'bg-slate-50 border-slate-100 text-slate-300'}`}>A</button>
                    </div>
                  </div>
                ))}
            </div>
          </div>
          <div className="fixed bottom-0 max-w-[450px] w-full p-8 bg-gradient-to-t from-white flex justify-center z-40">
          ...
          <div className="p-6">
             <div className="bg-white rounded-[3.5rem] p-12 shadow-2xl border border-slate-100 relative overflow-hidden">
                <div className="relative z-10 flex flex-col items-center">
                   <div className="w-24 h-24 mb-6 bg-red-800 rounded-3xl flex items-center justify-center"><BookOpen size={48} className="text-white" /></div>
                   <h3 className="text-2xl font-black text-red-900 mb-2">AttendX Development</h3>
                   <div className="w-16 h-1 bg-red-800 rounded-full mb-8"></div>
                   <p className="text-sm px-2 text-slate-600 font-medium leading-relaxed">
                   ...
                   <div className="mt-8 p-10 bg-red-50 rounded-[3rem] border border-red-100 shadow-inner">
                        ...
                        <p className="text-2xl font-black text-slate-950">HOD Sarvat Nizamani</p>
                   </div>
                </div>
             </div>
          </div>
