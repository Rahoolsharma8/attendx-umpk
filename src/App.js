 ...
 * Version: 22.0 (Strict WhatsApp File Share & Middle Scroll Fix)
 */
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const localRecords = localStorage.getItem('attendx_v22_final_db');
  ...
  const handleFinalize = () => {
    ...
    localStorage.setItem('attendx_v22_final_db', JSON.stringify(updatedRecords));
  ...
  // UPDATED: STRICT FILE SHARING FOR DOCUMENT RECOGNITION
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: `Attendance ${ext.toUpperCase()} File`,
          text: `Attendance Report for ${rec.subject}`
        });
        showToast("Share Menu Opened!");
      } else {
        // Direct WhatsApp Text Fallback
        const encoded = encodeURIComponent(formatReportContent(rec));
        window.open(`https://wa.me/?text=${encoded}`, '_blank');
      }
    } catch (e) {
      showToast("Error during file generation.");
    }
  };
  ...
  const deleteRecord = (key) => {
    ...
    localStorage.setItem('attendx_v22_final_db', JSON.stringify(updated));
  ...
  return (
    <div className="min-h-screen bg-slate-50 max-w-[450px] mx-auto flex flex-col font-sans relative overflow-hidden text-slate-900 shadow-2xl border-x border-slate-200 antialiased touch-pan-y">
      ...
      {selectedDept && !selectedProgramObj && (
        <div className="flex-1 flex flex-col bg-white overflow-y-auto pb-32 animate-in slide-in-from-right-10 duration-500 text-left text-left">
      ...
      {/* 4. MARKING SHEET (MIDDLE SCROLL FIXED) */}
      {selectedSem && (
        <div className="flex-1 flex flex-col overflow-hidden animate-in slide-in-from-right-10 duration-500 bg-slate-50">
          ...
          <div className="flex-1 overflow-y-auto p-5 pb-48 space-y-4 touch-auto overscroll-contain" style={{ WebkitOverflowScrolling: 'touch' }}>
            ...
            <div className="space-y-2 text-left min-h-[500px]">
          ...
          <div className="fixed bottom-0 max-w-[450px] w-full p-8 bg-gradient-to-t from-white via-white/95 to-transparent flex justify-center z-40 text-center">
             <button onClick={handleFinalize} className="w-full bg-[#800000] text-white py-5 rounded-[2.2rem] font-black shadow-2xl active:scale-95 transition-all uppercase tracking-widest text-xs flex items-center justify-center gap-3 border-b-4 border-black/20 text-center uppercase uppercase">
      ...
      {/* HISTORY TAB */}
      {activeTab === 'records' && (
        <div className="flex-1 flex flex-col bg-slate-50 overflow-y-auto pb-32 animate-in fade-in duration-500 text-left text-left">
              ...
              savedRecords.map((rec) => (
                <div key={rec.uniqueKey} className="bg-white p-5 rounded-[2.2rem] shadow-sm border border-slate-100 space-y-4 text-left">
                   <div className="flex justify-between items-start text-left text-left">
                      <div onClick={() => setViewingRecord(rec)} className="flex-1 cursor-pointer text-left text-left">
                        ...
                        <div className="flex flex-col gap-1 text-left">
                          ...
                          <p className="text-[9px] font-black text-slate-800 text-left text-left">Dates stored: {rec.sessions.length}</p>
                        </div>
                      </div>
                   ...
                   <button onClick={() => setViewingRecord(rec)} className="w-full bg-[#800000]/5 text-[#800000] py-3.5 rounded-xl font-black text-[9px] uppercase tracking-[0.2em] active:bg-[#800000] active:text-white transition-all shadow-sm text-center uppercase uppercase">OPEN MERGED SHEET</button>
                </div>
              ))
            )}
          </div>
          ...
          <div className="p-5 space-y-6 flex-1 text-center text-center">
             <div className="bg-gradient-to-br from-[#800000] to-[#4a0404] rounded-[2.5rem] p-10 text-white shadow-2xl text-center relative overflow-hidden">
                <div className="relative z-10 flex flex-col items-center">
                   ...
                   <p className="text-[9px] font-bold text-white/50 uppercase tracking-[0.3em] leading-relaxed text-center text-center">Intelligence Engine</p>
                </div>
             ...
             <div className="space-y-4 px-1 text-left text-left">
                <div className="bg-slate-50 rounded-[2.2rem] p-8 border border-slate-100 shadow-sm text-left relative overflow-hidden text-left">
                   ...
                   <h2 className="text-[18px] font-black text-slate-800 leading-tight uppercase tracking-tight mb-6 text-left text-left">INTELLIGENT CORE SYSTEMS</h2>
                   <div className="h-[1px] w-full bg-slate-200 mb-6 opacity-50 text-left"></div>
                   <p className="text-slate-400 text-[9px] font-black uppercase tracking-[0.2em] mb-4 text-left uppercase text-left">DEVELOPED BY</p>
                   <h2 className="text-[18px] font-black text-slate-800 leading-tight uppercase tracking-tight text-left text-left">DEPARTMENT OF <span className="text-[#800000]">COMPUTER SCIENCE</span></h2>
                   <div className="flex items-center gap-3 mt-5 bg-white p-3 rounded-2xl border border-slate-100 shadow-sm text-left text-left text-left">
                      <div className="w-10 h-10 bg-[#800000] rounded-xl flex items-center justify-center text-white text-center text-center"><UserCircle size={24}/></div>
                      <div className="text-left text-left text-left">
                         <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1 text-left text-left">Lead Developer</p>
                         <p className="font-black text-slate-900 text-base leading-none text-left uppercase text-left">Rahool</p>
                      </div>
                   </div>
                   <p className="font-bold text-slate-400 text-[10px] mt-6 uppercase tracking-widest text-left text-left">University of Mirpurkhas (UMPK)</p>
                </div>
                <div className="p-8 bg-white border border-slate-100 rounded-[2.2rem] flex items-center gap-6 shadow-sm active:bg-slate-50 transition-colors text-left text-left text-left text-left">
                   <div className="w-16 h-16 bg-red-50 text-[#800000] rounded-2xl flex items-center justify-center shadow-inner text-center text-center"><Award size={32} /></div>
                   <div className="text-left flex-1 text-left text-left">
                      ...
                      <h4 className="text-xl font-black text-slate-900 leading-none tracking-tighter text-left text-left text-left">Sarvat Nizamani</h4>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 leading-relaxed text-left uppercase text-left text-left">HEAD OF COMPUTER SCIENCE</p>
                   </div>
                </div>
                <div className="overflow-hidden py-5 border-y border-slate-100 relative bg-slate-50/50 rounded-2xl text-left text-left">
      ...
      {viewingRecord && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[600] flex items-center justify-center p-4 animate-in fade-in duration-300 text-left">
          <div className="bg-white w-[94%] max-w-[360px] rounded-[3rem] p-7 space-y-6 animate-in zoom-in-95 duration-200 shadow-2xl relative text-left text-left">
             <div className="flex justify-between items-center border-b border-slate-50 pb-5 text-left text-left text-left">
                <div className="text-left text-left text-left">
                   <h3 className="font-black text-2xl leading-none uppercase tracking-tighter text-slate-900 text-left text-left uppercase text-left">Sheet Preview</h3>
                   <p className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-2 text-left text-left">{viewingRecord.subject}</p>
                </div>
             ...
             <div className="bg-slate-50 p-6 rounded-[2.2rem] font-mono text-[12px] text-slate-700 leading-relaxed overflow-y-auto max-h-[300px] whitespace-pre-wrap border border-slate-100 text-left uppercase uppercase text-left">
                {formatReportContent(viewingRecord)}
             </div>
             <div className="grid grid-cols-1 gap-3.5 text-center">
                <button onClick={() => handleShareFile(viewingRecord, 'csv')} className="w-full bg-slate-900 text-white py-4.5 rounded-[1.8rem] font-black text-[11px] uppercase tracking-[0.1em] flex items-center justify-center gap-3 active:scale-95 transition-all shadow-xl text-center uppercase">
                  <FileSpreadsheet size={18}/> SHARE CSV AS DOCUMENT
                </button>
                <div className="grid grid-cols-2 gap-3 text-center">
                   <button onClick={() => handleDownloadDirect(viewingRecord, 'csv')} className="w-full bg-slate-100 text-slate-600 py-3.5 rounded-[1.5rem] font-black text-[10px] uppercase tracking-[0.1em] flex items-center justify-center gap-2 active:bg-slate-200 transition-all text-center">
                      <Download size={16}/> CSV
                   </button>
                   <button onClick={() => handleDownloadDirect(viewingRecord, 'txt')} className="w-full bg-slate-100 text-slate-600 py-3.5 rounded-[1.5rem] font-black text-[10px] uppercase tracking-[0.1em] flex items-center justify-center gap-2 active:bg-slate-200 transition-all text-center">
      ...
      {/* NAVIGATION BAR */}
      {!selectedSem && (
        <div className="fixed bottom-0 max-w-[450px] w-full bg-white/95 backdrop-blur-3xl border-t border-slate-100 px-10 py-6 flex justify-around items-center rounded-t-[3.2rem] shadow-xl z-100 text-center">
          <button onClick={() => {setActiveTab('home'); setSelectedDept(null); setSelectedProgramObj(null);}} className={`flex flex-col items-center gap-2 transition-all ${activeTab === 'home' ? 'text-[#800000] scale-110' : 'text-slate-300'} text-center`}>
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${activeTab === 'home' ? 'bg-red-50 shadow-sm' : 'bg-transparent'} text-center`}><Home size={26} fill={activeTab === 'home' ? "currentColor" : "none"} /></div>
            <span className="text-[9px] font-black uppercase tracking-[0.1em] text-center text-center">PORTAL</span>
          </button>
          <button onClick={() => setActiveTab('records')} className={`flex flex-col items-center gap-2 transition-all ${activeTab === 'records' ? 'text-[#800000] scale-110' : 'text-slate-300'} text-center`}>
            ...
            <span className="text-[9px] font-black uppercase tracking-[0.1em] text-center text-center text-center">RECORDS</span>
          </button>
          <button onClick={() => setActiveTab('info')} className={`flex flex-col items-center gap-2 transition-all ${activeTab === 'info' ? 'text-[#800000] scale-110' : 'text-slate-300'} text-center`}>
            ...
            <span className="text-[9px] font-black uppercase tracking-[0.1em] text-center text-center text-center text-center text-center">ABOUT</span>
          </button>
        </div>
      )}
