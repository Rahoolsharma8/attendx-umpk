...
import { 
  LayoutGrid, ArrowLeft, Home, History, Info, Search, 
  BookOpen, Monitor, Database, Cpu, Briefcase, Globe, Award,
  ChevronDown, FileText, Trash2, Share2, X,
  Heart, Activity, Bell, Clock, UserCircle, PlusCircle, Settings, Sparkles, Copy, ShieldCheck
} from 'lucide-react';
...
const DEPARTMENT_SUBJECT_MAPS = {
  'CS': {
    '1': ['Programming Fundamentals', 'Intro to ICT', 'English Comp.', 'Applied Physics', 'Accounting', 'Pakistan Studies'],
  ...
  'Commerce': {
    '1': ['Intro to Business', 'Micro Economics', 'Business Maths', 'Functional English', 'Islamic Studies', 'Accounting-I', 'Ethics'],
    '2': ['Business Statistics', 'Macro Economics', 'Computer Apps', 'Pakistan Studies', 'Financial Accounting', 'Business Comm.'],
    '3': ['Advanced Accounting-I', 'Business Law', 'Economic Issues', 'Inferential Stats', 'Business Finance', 'Psychology'],
    '4': ['Auditing', 'Cost Accounting', 'Advanced Accounting-II', 'Financial Institutions', 'Taxation', 'Technical Writing'],
    '5': ['Principles of Management', 'Principles of Marketing', 'Managerial Economics', 'Financial Mgmt', 'Sociology', 'Research Methods'],
    '6': ['Research Project Report', 'Logical Thinking', 'Risk Management', 'Managerial Accounting', 'HRM', 'Entrepreneurship'],
    '7': ['Corporate Finance', 'International Finance', 'Org Behaviour', 'Investment Mgmt', 'E-Commerce', 'Operations Mgmt'],
    '8': ['Strategic Management', 'Business Internship', 'Treasury Mgmt', 'Corporate Governance', 'Int Business', 'Specialized Financial Inst.']
  }
};

const STUDENT_LIST = Array.from({ length: 100 }, (_, i) => ({ id: i + 1, roll: (i + 1).toString() }));
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const localRecords = localStorage.getItem('attendx_v33_db');
  ...
  const handleAddCustomDept = () => {
    ...
    const newDept = {
      ...
      programs: [{ name: 'Bachelor’s Degree Programmes (4 Years)', sems: [1,2,3,4,5,6,7,8] }]
    };
  ...
  const handleFinalize = () => {
    ...
    const uniqueKey = `${selectedDept}_${selectedSem}_${selectedSubject.replace(/\s+/g, '_')}_${selectedBatch}`;
    ...
    if (existingIndex !== -1) {
      ...
      const deptData = STATIC_DEPARTMENTS[selectedDept] || customDepts.find(d => d.id === selectedDept);
      updatedRecords = [{ 
        uniqueKey, dept: deptData?.name || selectedDept, program: selectedProgramObj?.name,
    ...
    <div className="bg-white border-b border-slate-100 p-4 flex items-center justify-between sticky top-0 z-[1500] shadow-sm">
      <div className="flex items-center gap-4 text-slate-900">
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#800000] rounded-xl flex items-center justify-center shadow-md text-white">
             <BookOpen size={20} />
          </div>
          <h2 className="font-black text-slate-900 text-lg uppercase tracking-tight leading-none text-left">{title}</h2>
        </div>
      </div>
    </div>
  );
  ...
  if (view === 'splash') {
    return (
      <div className="h-screen bg-white flex flex-col items-center justify-center text-center p-10 overflow-hidden text-slate-900">
        <div className="w-24 h-24 mb-6 bg-[#800000] rounded-[2.2rem] flex items-center justify-center shadow-2xl animate-bounce text-white">
           <BookOpen size={48} />
        </div>
        <h1 className="text-4xl font-black tracking-tighter uppercase leading-none">AttendX</h1>
  ...
  return (
    <div className="min-h-screen h-screen bg-slate-50 max-w-[450px] mx-auto flex flex-col font-sans relative overflow-hidden text-slate-900 border-x border-slate-200 antialiased">
      {notification && (
        <div className="fixed top-8 left-1/2 -translate-x-1/2 z-[5000] bg-slate-900 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 animate-in slide-in-from-top-10">
      ...
      {activeTab === 'home' && (
        <div className="flex-1 flex flex-col h-full overflow-hidden bg-white animate-in fade-in">
          {!selectedDept ? (
            <div className="flex-1 flex flex-col overflow-y-auto pb-32">
              ...
              <div className="p-5 flex-1 text-left text-slate-900">
                ...
                <div className="grid grid-cols-2 gap-4 text-slate-900">
                  ...
                  <button onClick={() => setIsAddingDept(true)} className="bg-slate-50 border-2 border-dashed border-slate-200 p-6 rounded-[2rem] flex flex-col items-center justify-center gap-3 active:scale-95">
                  ...
                  <div className="bg-white p-6 rounded-[2.2rem] shadow-sm border border-slate-100 space-y-4 text-left">
                      <div className="space-y-1.5">
                        ...
                        <div className="relative text-slate-900 text-left">
                          {isManualSubject ? (
                            ...
                            <>
                              <select className="w-full p-4 bg-slate-50 rounded-xl appearance-none outline-none font-bold text-slate-800 text-sm border border-transparent text-left" value={selectedSubject} onChange={(e) => setSelectedSubject(e.target.value)}>
                                <option value="">Select Course...</option>
                                {(DEPARTMENT_SUBJECT_MAPS[selectedDept]?.[selectedSem] || DEPARTMENT_SUBJECT_MAPS['CS'][selectedSem] || []).map(sub => <option key={sub} value={sub}>{sub}</option>)}
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5 text-left text-slate-900">
                          ...
                          <input type="text" className="w-full p-4 bg-slate-50 rounded-xl border-none outline-none font-bold text-slate-800 text-xs" value={selectedBatch} onChange={(e) => setSelectedBatch(e.target.value)} />
                        </div>
                        <div className="space-y-1.5 text-left text-slate-900">
                          <label className="text-[9px] font-black text-[#800000] uppercase tracking-widest">DATE</label>
                          <input type="date" className="w-full p-4 bg-slate-50 rounded-xl border-none outline-none font-bold text-slate-800 text-xs" value={date} onChange={(e) => setDate(e.target.value)} />
                        </div>
                      </div>
                  </div>
                   ...
                   {STUDENT_LIST.filter(s => s.roll.includes(searchQuery)).map(s => (
                      <div key={s.id} className={`bg-white p-4 rounded-[1.8rem] border transition-all flex items-center justify-between shadow-sm ${attendance[s.id] === 'Present' ? 'border-emerald-200 bg-emerald-50/20' : attendance[s.id] === 'Absent' ? 'border-rose-200 bg-rose-50/20' : 'border-slate-100'}`}>
                        ...
                        <div className="flex gap-2.5">
                           <button onClick={() => setAttendance(p => ({ ...p, [s.id]: 'Present' }))} className={`w-12 h-12 rounded-full font-black text-sm border-2 active:scale-90 ${attendance[s.id] === 'Present' ? 'bg-emerald-600 border-emerald-700 text-white shadow-sm' : 'bg-slate-50 border-slate-100 text-slate-300'}`}>P</button>
                           <button onClick={() => setAttendance(p => ({ ...p, [s.id]: 'Absent' }))} className={`w-12 h-12 rounded-full font-black text-sm border-2 active:scale-90 ${attendance[s.id] === 'Absent' ? 'bg-rose-600 border-rose-700 text-white shadow-sm' : 'bg-slate-50 border-slate-100 text-slate-300'}`}>A</button>
                        </div>
                      </div>
                   ))}
                </div>
          ...
          <div className="p-5 space-y-4 flex-1">
            {savedRecords.length === 0 ? (
              <div className="text-center py-24 opacity-20 flex flex-col items-center">
                ...
                <p className="font-bold text-[10px] uppercase tracking-widest">No Logs Stored</p>
              </div>
            ) : (
             ...
             <div className="space-y-4 text-left">
                ...
                <div className="p-6 bg-white border border-slate-100 rounded-[2.2rem] flex items-center gap-5 shadow-sm text-slate-900">
                   ...
                   <div>
                      ...
                      <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">HEAD OF CS DEPT, UMPK</p>
                   </div>
                </div>

                <div className="overflow-hidden py-4 bg-slate-50/50 border-y border-slate-100 rounded-xl shadow-inner">
                  <div className="flex animate-marquee-final will-change-transform">
                    <MarqueeContent />
                    <MarqueeContent />
                  </div>
                </div>
      ...
      {/* REPORT MODAL */}
      {viewingRecord && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[4500] flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white w-[94%] max-w-[360px] rounded-[3rem] p-7 space-y-6 animate-in zoom-in-95 shadow-2xl relative flex flex-col max-h-[85vh]">
             <div className="flex justify-between items-center border-b border-slate-50 pb-5">
                <div className="text-left text-slate-900">
                   <h3 className="font-black text-xl leading-none uppercase tracking-tighter">Report Preview</h3>
      ...
      {isAddingDept && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[7000] flex items-center justify-center p-6 animate-in fade-in">
           <div className="bg-white w-full max-w-[340px] rounded-[3rem] p-8 space-y-6 animate-in zoom-in-95 shadow-2xl text-slate-900">
              ...
              <input type="text" placeholder="Name..." className="w-full p-4 bg-slate-50 border-none rounded-2xl outline-none font-black text-slate-800 text-sm focus:ring-2 focus:ring-[#800000]/20 shadow-inner" value={newDeptName} onChange={(e) => setNewDeptName(e.target.value)} />
              <div className="flex gap-3">
                 <button onClick={() => setIsAddingDept(false)} className="flex-1 py-4 rounded-2xl font-black text-[10px] uppercase bg-slate-100 text-slate-500 active:scale-95">CANCEL</button>
                 <button onClick={handleAddCustomDept} className="flex-1 py-4 rounded-2xl font-black text-[10px] uppercase bg-[#800000] text-white shadow-xl active:scale-95">ADD</button>
              </div>
           </div>
        </div>
      )}
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .animate-marquee-final { 
          display: flex; 
          animation: marquee-final 30s linear infinite; 
          width: max-content;
        }
        ::-webkit-scrollbar { width: 0px; background: transparent; }
        body { overscroll-behavior: none; -webkit-tap-highlight-color: transparent; height: 100vh; overflow: hidden; position: fixed; width: 100%; }
        input, select, .overflow-y-auto, textarea { -webkit-user-select: text !important; user-select: text !important; }
